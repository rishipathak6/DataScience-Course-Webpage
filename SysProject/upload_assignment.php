<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $about = $_POST["texty"];
        $selqry = mysqli_query($db,"SELECT * FROM `UploadAssignment`");
        $curr = mysqli_num_rows($selqry);
        $newfilename = $filename.$curr.'.'. end($temp);
      	move_uploaded_file($_FILES['path']['tmp_name'], "Assigments/pdf/".$newfilename);
        $qry = mysqli_query($db,"INSERT INTO `UploadAssignment`(`path`,`about`)VALUES('".$newfilename."','".$about."')");
      	if($qry){
      		echo "File Succesfully Uploaded";
      	}else{
      		echo "File Uploading Failed! Please Try Again";
      	}
    }
    ?>
...
-->
    ...
    <section class="section py-8" style="background-image: url(assets/img/upload.jpg)" data-overlay="5">
        ...
        <div class="container">
          <h2 class="text-white text-center lead-6">Upload the Assignment</h2>
          <p class="text-white text-center opacity-80 lead-2">These assignments really help the students</p>
          <br>
